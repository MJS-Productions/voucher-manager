...
The Pool overview always exposes a secondary Danger Zone path. The Danger Zone links to a dedicated confirmation view for deleting available codes; that view displays the affected count and requires explicit acknowledgement. Destructive execution is POST-only at the admin boundary, protected by `manage_options` and WordPress nonces. Full deletion additionally requires the exact pool name.
