<?php
/**
 * Pool overview.
 *
 * @package VoucherManager
 */

/** @var array<int,array{pool:\VoucherManager\Domain\Pool\Pool,total:int,available:int,assigned:int}> $pool_rows */

declare(strict_types=1);

use VoucherManager\Admin\Capabilities;
use VoucherManager\Admin\PoolViewModel;

if ( ! defined( 'ABSPATH' ) ) {
	exit;
}

$notice = isset( $_GET['vm_notice'] )
	? sanitize_key( wp_unslash( $_GET['vm_notice'] ) )
	: '';

$messages = array(
	'created'        => __( 'The first pool has been created.', 'mjs-productions-voucher-manager' ),
	'updated'        => __( 'Pool updated.', 'mjs-productions-voucher-manager' ),
	'deleted'        => __( 'Pool deleted.', 'mjs-productions-voucher-manager' ),
	'status'         => __( 'Pool status updated.', 'mjs-productions-voucher-manager' ),
	'invalid'        => __( 'Please enter a pool name.', 'mjs-productions-voucher-manager' ),
	'error'          => __( 'The operation could not be completed.', 'mjs-productions-voucher-manager' ),
	'available_deleted' => __( 'Available One-Time Codes permanently deleted.', 'mjs-productions-voucher-manager' ),
);

$view_model = new PoolViewModel();

$can_manage_pools    = current_user_can( Capabilities::MANAGE_POOLS );
$can_delete_pools    = current_user_can( Capabilities::DELETE_POOLS );
$can_import_codes    = current_user_can( Capabilities::IMPORT_CODES );
$can_distribute      = current_user_can( Capabilities::DISTRIBUTE_CODES );
$can_view_inventory  = current_user_can( Capabilities::VIEW_INVENTORY );
?>
<div class="wrap voucher-manager">
	<header class="voucher-manager__header">
		<div>
			<h1><?php echo esc_html__( 'Pools', 'mjs-productions-voucher-manager' ); ?></h1>
			<p><?php echo esc_html__( 'Organize One-Time Codes into pools for controlled distribution.', 'mjs-productions-voucher-manager' ); ?></p>
		</div>
		<?php if ( $can_manage_pools ) : ?>
			<a class="button button-primary" href="<?php echo esc_url( add_query_arg( array( 'page' => 'voucher-manager-pools', 'action' => 'new' ), admin_url( 'admin.php' ) ) ); ?>">
				<?php echo esc_html__( 'Create Pool', 'mjs-productions-voucher-manager' ); ?>
			</a>
		<?php endif; ?>
	</header>

	<?php if ( isset( $messages[ $notice ] ) ) : ?>
		<div class="notice <?php echo in_array( $notice, array( 'invalid', 'error' ), true ) ? 'notice-error' : 'notice-success'; ?> is-dismissible">
			<p><?php echo esc_html( $messages[ $notice ] ); ?></p>
		</div>
	<?php endif; ?>

	<?php if ( empty( $pool_rows ) ) : ?>
		<section class="voucher-manager__card voucher-manager__pool-empty">
			<span class="dashicons dashicons-tickets-alt" aria-hidden="true"></span>
			<h2><?php echo esc_html__( 'Create the first pool', 'mjs-productions-voucher-manager' ); ?></h2>
			<p><?php echo esc_html__( 'A pool groups One-Time Codes that belong to the same campaign, product or distribution workflow.', 'mjs-productions-voucher-manager' ); ?></p>
			<?php if ( $can_manage_pools ) : ?>
				<a class="button button-primary" href="<?php echo esc_url( add_query_arg( array( 'page' => 'voucher-manager-pools', 'action' => 'new' ), admin_url( 'admin.php' ) ) ); ?>">
					<?php echo esc_html__( 'Create Pool', 'mjs-productions-voucher-manager' ); ?>
				</a>
			<?php endif; ?>
		</section>
	<?php else : ?>
		<section class="voucher-manager__pool-grid" aria-label="<?php echo esc_attr__( 'Pool overview', 'mjs-productions-voucher-manager' ); ?>">
			<?php foreach ( $pool_rows as $row ) : ?>
				<?php
				$pool       = $row['pool'];
				$pool_id    = (int) $pool->id();
				$state      = $view_model->inventory_state( $pool, $row['available'] );
				$edit_url   = add_query_arg(
					array(
						'page'    => 'voucher-manager-pools',
						'action'  => 'edit',
						'pool_id' => $pool_id,
					),
					admin_url( 'admin.php' )
				);
				$import_url = add_query_arg(
					array(
						'page'    => 'voucher-manager-import',
						'pool_id' => $pool_id,
					),
					admin_url( 'admin.php' )
				);
				$distribution_url = add_query_arg(
					array(
						'page'    => 'voucher-manager-distribution',
						'pool_id' => $pool_id,
					),
					admin_url( 'admin.php' )
				);
				$inventory_url = add_query_arg(
					array(
						'page'    => 'voucher-manager-inventory',
						'pool_id' => $pool_id,
					),
					admin_url( 'admin.php' )
				);
				$toggle_url = wp_nonce_url(
					add_query_arg(
						array(
							'action'  => 'voucher_manager_toggle_pool',
							'pool_id' => $pool_id,
						),
						admin_url( 'admin-post.php' )
					),
					'voucher_manager_toggle_pool_' . $pool_id
				);
				$danger_url = add_query_arg( array( 'page' => 'voucher-manager-pools', 'action' => 'danger-zone', 'pool_id' => $pool_id ), admin_url( 'admin.php' ) );
				?>
				<article class="voucher-manager__pool-card voucher-manager__pool-card--<?php echo esc_attr( $state ); ?>">
					<div class="voucher-manager__pool-card-header">
						<div>
							<h2>
								<?php if ( $can_manage_pools ) : ?>
									<a href="<?php echo esc_url( $edit_url ); ?>"><?php echo esc_html( $pool->name() ); ?></a>
								<?php else : ?>
									<?php echo esc_html( $pool->name() ); ?>
								<?php endif; ?>
							</h2>
							<?php if ( '' !== $pool->description() ) : ?>
								<p><?php echo esc_html( $pool->description() ); ?></p>
							<?php else : ?>
								<p class="voucher-manager__muted"><?php echo esc_html__( 'No description added.', 'mjs-productions-voucher-manager' ); ?></p>
							<?php endif; ?>
						</div>
						<span class="voucher-manager__inventory-badge voucher-manager__inventory-badge--<?php echo esc_attr( $state ); ?>">
							<?php echo esc_html( $view_model->inventory_label( $pool, $row['available'] ) ); ?>
						</span>
					</div>

					<div class="voucher-manager__pool-inventory">
						<div>
							<strong><?php echo esc_html( number_format_i18n( $row['available'] ) ); ?></strong>
							<span><?php echo esc_html_x( 'Available', 'One-Time Code status', 'mjs-productions-voucher-manager' ); ?></span>
						</div>
						<div>
							<strong><?php echo esc_html( number_format_i18n( $row['assigned'] ) ); ?></strong>
							<span><?php echo esc_html__( 'Distributed', 'mjs-productions-voucher-manager' ); ?></span>
						</div>
						<div>
							<strong><?php echo esc_html( number_format_i18n( $row['total'] ) ); ?></strong>
							<span><?php echo esc_html__( 'Total One-Time Codes', 'mjs-productions-voucher-manager' ); ?></span>
						</div>
					</div>

					<p class="voucher-manager__pool-hint">
						<?php echo esc_html( $view_model->inventory_hint( $pool, $row['available'] ) ); ?>
					</p>

					<div class="voucher-manager__pool-actions">
						<?php if ( $can_distribute && $pool->is_active() && 0 < $row['available'] ) : ?>
							<a class="button button-primary" href="<?php echo esc_url( $distribution_url ); ?>">
								<?php echo esc_html__( 'Distribute Code', 'mjs-productions-voucher-manager' ); ?>
							</a>
						<?php elseif ( $can_import_codes ) : ?>
							<a class="button button-primary" href="<?php echo esc_url( $import_url ); ?>">
								<?php echo esc_html__( 'Import Codes', 'mjs-productions-voucher-manager' ); ?>
							</a>
						<?php endif; ?>
						<?php if ( $can_view_inventory ) : ?>
							<a class="button" href="<?php echo esc_url( $inventory_url ); ?>">
								<?php echo esc_html__( 'View inventory', 'mjs-productions-voucher-manager' ); ?>
							</a>
						<?php endif; ?>
						<?php if ( $can_manage_pools ) : ?>
							<a class="button" href="<?php echo esc_url( $edit_url ); ?>">
								<?php echo esc_html__( 'Edit', 'mjs-productions-voucher-manager' ); ?>
							</a>
						<?php endif; ?>
						<?php if ( $can_manage_pools || $can_delete_pools ) : ?>
							<div class="voucher-manager__pool-secondary-actions">
								<?php if ( $can_manage_pools ) : ?>
									<a href="<?php echo esc_url( $toggle_url ); ?>">
										<?php echo esc_html( $pool->is_active() ? __( 'Deactivate', 'mjs-productions-voucher-manager' ) : __( 'Activate', 'mjs-productions-voucher-manager' ) ); ?>
									</a>
								<?php endif; ?>
								<?php if ( $can_delete_pools ) : ?>
									<a class="button-link-delete" href="<?php echo esc_url( $danger_url ); ?>"><?php echo esc_html__( 'Danger Zone', 'mjs-productions-voucher-manager' ); ?></a>
								<?php endif; ?>
							</div>
						<?php endif; ?>
					</div>
				</article>
			<?php endforeach; ?>
		</section>
	<?php endif; ?>
</div>
