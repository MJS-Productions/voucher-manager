...
<div class="wrap voucher-manager">
	...
	<p class="description"><?php echo esc_html__( 'Assign one available One-Time Code from an active pool. Each successful distribution immediately marks the code as assigned.', 'mjs-productions-voucher-manager' ); ?></p>

	<?php if ( 'replayed' === $notice ) : ?>
		<div class="notice notice-warning inline"><p><?php echo esc_html__( 'This distribution request was already used or expired. No additional One-Time Code was distributed.', 'mjs-productions-voucher-manager' ); ?></p></div>
	<?php endif; ?>
	...
	<?php if ( is_array( $result ) ) : ?>
		<?php if ( ! empty( $result['success'] ) && is_string( $result['code'] ?? null ) ) : ?>
			...
			<div class="voucher-manager__distribution-result voucher-manager__distribution-result--<?php echo esc_attr( $view->result_tone( $remaining ) ); ?>">
				...
				<p class="voucher-manager__result-guidance"><?php echo esc_html__( 'Copy the One-Time Code now and deliver it through the intended channel. For privacy, the complete value is shown only in this one-time result.', 'mjs-productions-voucher-manager' ); ?></p>
				<div class="voucher-manager__code-result" aria-label="<?php echo esc_attr__( 'Distributed One-Time Code', 'mjs-productions-voucher-manager' ); ?>">
					...
					<button type="button" class="button button-secondary" id="vm-copy-distributed-code" data-copy-label="<?php echo esc_attr__( 'Copy code', 'mjs-productions-voucher-manager' ); ?>" data-copied-label="<?php echo esc_attr__( 'Copied', 'mjs-productions-voucher-manager' ); ?>">
						<?php echo esc_html__( 'Copy code', 'mjs-productions-voucher-manager' ); ?>
					</button>
				</div>
				...
				<p>
					<a class="button button-primary" href="<?php echo esc_url( add_query_arg( array( 'page' => 'voucher-manager-distribution', 'pool_id' => absint( $result['pool_id'] ?? 0 ) ), admin_url( 'admin.php' ) ) ); ?>">
						<?php echo esc_html__( 'Distribute another One-Time Code', 'mjs-productions-voucher-manager' ); ?>
					</a>
				</p>
			</div>
			<script>
	...
	<div class="voucher-manager__distribution-grid">
		<div class="voucher-manager__card voucher-manager__form">
			<h2><?php echo esc_html__( 'Distribute next available One-Time Code', 'mjs-productions-voucher-manager' ); ?></h2>
			<p><?php echo esc_html__( 'Choose a pool with available inventory. Voucher Manager automatically assigns the next available One-Time Code. Concurrent requests never receive the same code.', 'mjs-productions-voucher-manager' ); ?></p>
			...
			<?php else : ?>
				<form class="voucher-manager__distribution-form" method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
					...
					<p>
						<label for="vm-distribution-pool"><strong><?php echo esc_html_x( 'Pool', 'Distribution form field label', 'mjs-productions-voucher-manager' ); ?></strong></label>
					...
					<p class="description"><?php echo esc_html__( 'One-Time Code assignment occurs immediately after confirmation.', 'mjs-productions-voucher-manager' ); ?></p>
					<?php submit_button( __( 'Distribute Code', 'mjs-productions-voucher-manager' ) ); ?>
				</form>
			<?php endif; ?>
		</div>

		<div class="voucher-manager__card">
			<h2><?php echo esc_html__( 'Distribution safety', 'mjs-productions-voucher-manager' ); ?></h2>
			<ul class="voucher-manager__guidance-list">
				<li><?php echo esc_html__( 'Only active pools with available inventory can be selected.', 'mjs-productions-voucher-manager' ); ?></li>
				<li><?php echo esc_html__( 'A successful distribution changes the One-Time Code from available to assigned immediately.', 'mjs-productions-voucher-manager' ); ?></li>
				<li><?php echo esc_html__( 'One-Time Code values are not stored in Activity history.', 'mjs-productions-voucher-manager' ); ?></li>
				<li><?php echo esc_html__( 'Refreshing the page does not distribute another One-Time Code. Every distribution requires a new form submission.', 'mjs-productions-voucher-manager' ); ?></li>
			</ul>
		</div>
	</div>
	<?php endif; ?>
