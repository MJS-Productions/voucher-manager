...
$assert(
	is_string( $readme ) && str_contains( $readme, 'Release candidate:** `' . $expected . '` — The Translation Foundation' ),
	'README must identify the reviewed release candidate.'
);

$assert(
	is_string( $changelog ) && str_contains( $changelog, '## ' . $expected . ' - 2026-07-16 — The Translation Foundation' ),
	'Changelog must contain the reviewed release section.'
);

$assert(
	is_string( $release )
	&& str_contains( $release, '# Voucher Manager ' . $expected . ' — The Translation Foundation' )
...
$assert(
	! file_exists( $root . '/RELEASE-0.9.0-alpha.md' ),
...
echo "Release identity OK: 0.9.1-alpha is consistent across plugin, README, changelog, release notes and final gate.\n";
