...
$readiness = file_get_contents( $root . '/docs/SPRINT_6_RELEASE_READINESS.md' );
...
$assert(
	is_string( $readme ) && str_contains( $readme, 'Release candidate:** `' . $expected . '` — The First Experience' ),
	'README must identify the reviewed release candidate.'
);

$assert(
	is_string( $changelog ) && str_contains( $changelog, '## ' . $expected . ' - 2026-07-13 — The First Experience' ),
	'Changelog must contain the reviewed release section.'
);

$assert(
	is_string( $release )
	&& str_contains( $release, '# Voucher Manager ' . $expected . ' — The First Experience' )
...
$assert(
	! file_exists( $root . '/RELEASE-0.5.0-alpha.md' ),
	'The root release note must describe the current candidate rather than the previous release.'
);

$assert(
	is_string( $readiness )
	&& str_contains( $readiness, 'selected `0.6.0-alpha`' )
	&& str_contains( $readiness, 'Confirm WordPress reports `0.6.0-alpha`' ),
	'Release-readiness documentation must reflect the selected version.'
);

echo "Release identity OK: 0.6.0-alpha is consistent across plugin, README, changelog, release notes and final gate.\n";
