...
$assert( '4 One-Time Codes remain available in this pool.' === $view->remaining_message( 4 ), 'Success result must explain remaining inventory.' );
$assert( 'This pool is now empty. Import more One-Time Codes before the next distribution.' === $view->remaining_message( 0 ), 'Final distribution must guide the administrator to import.' );
...
$assert( is_string( $template ) && str_contains( $template, 'Assigned One-Time Code' ) && str_contains( $template, 'shown only in this one-time result' ), 'Success state must clearly present the one-time voucher result.' );
...
$assert( str_contains( $template, 'No One-Time Codes are ready to distribute' ) && str_contains( $template, 'voucher-manager-import' ), 'Empty inventory must show a guided import action.' );
$assert( str_contains( $template, 'Every distribution requires a new form submission' ), 'Distribution safety guidance must explain refresh behavior.' );
$assert(
