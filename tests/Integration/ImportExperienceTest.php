...
$assert( str_contains( $admin, "isset( \$_POST['import_id'] )" ) && str_contains( $admin, 'check_admin_referer' ) && str_contains( $admin, 'guard( Capabilities::ROLLBACK_IMPORTS )' ), 'Rollback execution must retain POST, nonce and dedicated rollback capability protection.' );
$assert( str_contains( $admin, 'guard( Capabilities::IMPORT_CODES )' ) && str_contains( $template, 'current_user_can( Capabilities::ROLLBACK_IMPORTS )' ), 'Import and rollback must remain separate capability boundaries in both controller and UI.' );
